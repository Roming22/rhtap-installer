  ...
  command:
    ...
    {{ if and (index .Values "pipelines") (index .Values "pipelines" "pipelines-as-code") }}
      ...
      if [ "$(kubectl get secret "$CHART-pipelines-secret" -o name --ignore-not-found | wc -l)" = "0" ]; then
        echo -n "."
        WEBHOOK_SECRET="{{ default "$(openssl rand -hex 20)" (index .Values "pipelines" "pipelines-as-code" "webhook-secret") | replace "$" "\\$" }}"
      ...
      if [ "$(kubectl get secret -n "$PIPELINES_NAMESPACE" "pipelines-as-code-secret" -o name --ignore-not-found | wc -l)" = "0" ]; then
        ...
        {{ if (index .Values "pipelines" "pipelines-as-code" "github") }}
